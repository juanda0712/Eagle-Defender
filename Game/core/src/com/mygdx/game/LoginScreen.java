...
public class LoginScreen implements Screen {//QuestionsForm2.DialogCallback
        ...
        for (User2 user : data) {
            if (username.equals(user.getUsername()) && checkPassword(password, user.getPassword())) {
                ...
                if (user1 == null) {
                    ...
                    game.changeScreen(new GameScreen(game, user2Manager, user1, user, countersBarriers, spotifyReference, 0, 0, 0, new ConsoleConn()));
    ...
    @Override
    public void dispose() {
        if (faceRecognitionActor != null) {
            faceRecognitionActor.remove();
            faceRecognitionActor = null;
        }
        stage.dispose();
    }
